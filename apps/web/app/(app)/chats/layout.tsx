...
function ChatsFrame({ children }: { children: React.ReactNode }) {
  ...
  const showMainOnMobile = selectedId !== null || search.get("new") === "1";

  // Mobile: the shell adds a 3.5rem top bar and a 4rem bottom tab bar.
  return (
    <div className="flex h-[calc(100dvh-7.5rem)] min-h-0 w-full overflow-hidden bg-bg md:h-dvh">
      <aside
...
export default function ChatsLayout({ children }: { children: React.ReactNode }) {
  return (
    <Suspense fallback={<div className="h-[calc(100dvh-7.5rem)] md:h-dvh" />}>
