...
export function AgentBubble({ message, name }: { message: ConversationMessage; name: string }) {
  const text = messageText(message);
  return (
    <div data-testid="agent-message" className="flex items-end gap-2.5">
      <Avatar name={name} size="sm" className="mb-5" />
      <div className="max-w-[min(85%,40rem)]">
...
export function WorkCard({ message, name }: { message: ConversationMessage; name: string }) {
  ...
  return (
    <div data-testid="work-card" className="flex items-start gap-2.5">
...
export function WorkingIndicator({ status, name }: { status: LiveStatus; name: string }) {
      ...
      >
        <div className="mx-auto flex max-w-3xl flex-col gap-4">
          {items.map((item) => {
            ...
            if (isWorkCard(message)) return <WorkCard key={item.id} message={message} name={name} />;
            return <AgentBubble key={item.id} message={message} name={name} />;
          })}
          ...
          {liveStatus ? <WorkingIndicator status={liveStatus} name={agentName} /> : null}
