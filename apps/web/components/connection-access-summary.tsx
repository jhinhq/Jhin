      ...
      {summary.agents.map((agent) => (
        <article key={agent.agent_id} className="rounded-xl border border-line bg-raised px-4 py-3">
          <header className="flex min-w-0 flex-wrap items-center gap-2">
            <strong className="min-w-0 break-words text-sm font-semibold text-ink">{agent.agent_name}</strong>
            <Badge tone={agent.authorized ? "ok" : "danger"}>
              {agent.authorized ? "Authorized" : "Not authorized"}
            </Badge>
          </header>
          {agent.authorized_tool_names.length > 0 ? (
            <p className="mt-1 text-xs text-dim">{agent.authorized_tool_names.length} {agent.authorized_tool_names.length === 1 ? "tool" : "tools"} authorized on this app.</p>
          ) : null}
          <details className="mt-2">
            <summary className={`cursor-pointer rounded-md text-xs font-medium text-dim ${focusRing}`}>Advanced grant details</summary>
            {agent.authorized_tool_names.length > 0 ? <p className="mt-2 break-words text-xs text-dim">{agent.authorized_tool_names.join(", ")}</p> : null}
            <ul className="mt-2 space-y-2">
              {agent.grants.map((grant) => (
                <li key={grant.grant_id} className="text-xs">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <code className="min-w-0 break-words font-mono text-xs text-ink">{grant.capability}</code>
                    <Badge tone={grant.effect === "allow" ? "ok" : "danger"}>{grant.effect}</Badge>
                  </div>
                  <p className="mt-1 break-words text-faint">{formatScope(grant.scope)}</p>
                  {grant.eligibility_reason ? (
                    <p className="mt-1 break-words text-warn">{grant.eligibility_reason}</p>
                  ) : null}
                </li>
              ))}
            </ul>
