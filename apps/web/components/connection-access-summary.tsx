...
export function ConnectionAccessSummary({ summary }: { summary: ConnectionAccessSummaryOut }) {
  if (summary.agents.length === 0) {
    return <p className="text-xs text-faint">No agents have grants relevant to this connection.</p>;
  }
  return (
    <div className="space-y-3">
      ...
      {summary.agents.map((agent) => (
        <article key={agent.agent_id} className="rounded-lg border border-line bg-raised px-3 py-2.5">
          <header className="flex items-center gap-2">
            <strong className="text-sm">{agent.agent_name}</strong>
          ...
          <details className="mt-2">
            <summary className="cursor-pointer text-xs font-medium text-dim">Advanced grant details</summary>
