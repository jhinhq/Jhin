...
/** The reference's prose is markdown. Parsing and rendering it now lives in
 * `lib/markdown` / `components/markdown`, shared with the chat transcript so
 * that one link allow-list covers both. Re-exported here because the document
 * readers above and the prose belong to the same public surface. */
export { parseInline, parseMarkdown, safeHref } from "./markdown";
export type { Block, Inline } from "./markdown";
